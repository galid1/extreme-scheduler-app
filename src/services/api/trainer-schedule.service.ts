...
import {
    ...
    UpdateAutoSchedulingResultLinesApiRequest,
    UpdateAutoSchedulingResultLinesApiResponse,
} from '../../types/api';
...
class TrainerScheduleService {
    ...
    /**
     * 자동 스케줄링 결과 시간 수정
     * @param request 수정할 스케줄 라인 목록
     */
    async updateAutoSchedulingResultLines(
        request: UpdateAutoSchedulingResultLinesApiRequest
    ): Promise<UpdateAutoSchedulingResultLinesApiResponse> {
        return apiClient.put<UpdateAutoSchedulingResultLinesApiResponse>(
            '/api/v1/trainers/schedules/auto-scheduling/lines',
            request
        );
    }
